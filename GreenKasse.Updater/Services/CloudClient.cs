...
public sealed class CloudClient
{
    ...
    public CloudClient(IConfiguration cfg)
    {
        
        _cfg = cfg; 
        var baseUrl = cfg["Cloud:BaseUrlDev"] 
                      ?? throw new InvalidOperationException("Missing Cloud:BaseUrlDev");
    ...
    public async Task<(string path, string sha256)> DownloadAsync(string relativeUrl, string finalPath, CancellationToken ct)
    {
        ...
        await using (var fs = new FileStream(part, FileMode.Create, FileAccess.Write, FileShare.None, 81920, true))
            await resp.Content.CopyToAsync(fs, ct);

        using var sha = SHA256.Create();
        await using var f2 = File.OpenRead(part);
        var hex = Convert.ToHexString(await sha.ComputeHashAsync(f2, ct)).ToLowerInvariant();
